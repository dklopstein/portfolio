...
function displayStats() {
  ...
  // Add most common day of week commit
  let daysOfWeek = [
    "Sunday",
    "Monday",
    "Tuesday",
  ...
  let days = d3.group(commits, (d) => d.date.getDay());
  let mostCommonDay = d3.sort(
    days.entries(),
    ([keyA, valueA], [keyB, valueB]) => valueB.length - valueA.length
  )[0][0];

  dl.append("dt").text("MOST COMMITS ON");
  dl.append("dd").text(daysOfWeek[mostCommonDay]);
...
function updateTooltipContent(commit) {
  ...
  time.textContent = commit.datetime.toTimeString().split(" ")[0];
